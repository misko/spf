...
python simple_train_filter.py -d /mnt/md0/spf/2d_wallarray_v2_data/june_fix/*.zarr --precompute-cache ~/precompute_cache_chunk16_fresh/ \
	--act leaky --skip-qc --workers 5 --batch $batch --device cuda --wandb-project projectspf_aug23_v3_discrete --lr $lr \
	...
	--tformer-layers 8 --only-beamnet --beam-net-hidden $hid --beam-net-depth $layer --steps $step --weight-decay $wd --beam-norm-type ${norm} \
	--beam-type discrete
done
done
done
