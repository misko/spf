...
## Pipeline — how this appendix was produced, and where every file lives

The whole sequence is committed as one script,
[`tools/run_pipeline.sh`](tools/), which runs four steps: **merge → precompute →
figures → metrics**. It reads the recorded captures **read-only**; nothing is ever
written back into the raw capture directory.

```bash
# reproduce this entire appendix (defaults are the Aug-1 paths below)
data_collection/rover/rover_v3.1/field_reports/tools/run_pipeline.sh

# or run individual steps
STEPS=1,2 tools/run_pipeline.sh      # merge + precompute only
STEPS=4   tools/run_pipeline.sh      # just recompute the metrics table
```

### Step-by-step: script, command, and output location

| # | Step | Script | Output location | Size |
|---|---|---|---|---|
| 0 | Raw capture (input, immutable) | *(collector on the rover)* | `/mnt/qnap01/mouse9911/rovers_august_2026/aug1/` — `*.zarr[.tmp]` + `*.yaml[.tmp]`, one per launch | 204 GB / 30 stores |
| 1 | **Merge** TX↔RX → ground truth | [`spf/scripts/v7_tx_rx_merge.py`](../../../../spf/scripts/v7_tx_rx_merge.py) | `…/rovers_august_2026/merged/<rx>.<tx>.zarr` + `.yaml` | 113 GB / 14 datasets |
| 2 | **Precompute** segmentation + beamformer | [`spf/scripts/segment_zarr.py`](../../../../spf/scripts/segment_zarr.py) | `/mnt/qnap01/mouse9911/precomputed/precompute_cache_3p7/` — `<name>_segmentation_nthetas65.pkl` + `.yarr` | 2.0 GB / 14 datasets |
| 3 | **Figures** | [`tools/generate_figures.sh`](tools/) → `zarr_summary_figure.py`, `merged_df_figure.py` | [`2026_08_01_figures/`](2026_08_01_figures/) — `<capture>.png`, `df_<merged>.png` | 21 MB / 47 figures |
| 4 | **Metrics** (the DF table above) | [`tools/df_metrics.py`](tools/) | [`2026_08_01_metrics.json`](2026_08_01_metrics.json) | per-dataset R / MAE / NaN / signal |
...
**1 — Merge.** Pairs every emitter capture against every receiver capture; pairs that
do not overlap in GPS time (or cannot be read) are skipped in ~2 s each, so this
doubles as the overlap map. Add `--dry-run` to get the map without copying any IQ.
...
**2 — Precompute.** Builds the segmentation/beamformer cache the DF figures and
metrics both read.

```bash
$PY spf/scripts/segment_zarr.py -i "$AUG"/merged/*.zarr -c "$CACHE" --gpu -p 8
```

**3 — Figures.**

```bash
PY=$PY DATE=2026_08_01 RAW="$AUG"/aug1 MERGED="$AUG"/merged CACHE="$CACHE" \
  data_collection/rover/rover_v3.1/field_reports/tools/generate_figures.sh
```

**4 — Metrics.** Regenerates the DF quality table; `--markdown` prints it in the exact
form used above.

```bash
$PY data_collection/rover/rover_v3.1/field_reports/tools/df_metrics.py \
  "$AUG"/merged "$CACHE" \
  data_collection/rover/rover_v3.1/field_reports/2026_08_01_metrics.json --markdown
```

### Notes that matter when re-running

- **Cache dirs must be version-scoped.** Cache filenames encode the dataset name and
  `nthetas` but **not** the segmentation version, so two versions of the same dataset
  collide silently. Current version is `3.7` (`spf/utils.py`); hence `…_3p7`. Do not
  pin an older version when loading — `v5spfdataset` will raise on a mismatch.
- **Precompute locally, then publish.** `run_pipeline.sh` stages the cache under
  `~/spf_cache/` and rsyncs it to the network path: much faster, and it avoids writing
  LMDB stores over NFS.
- **`--min-timesteps 300`**, not the default 500. At 500 the two straddle-tails (349
  and 393 snapshots, where a receiver ran across an emitter changeover) are silently
  dropped.
- **Raw data is immutable.** Every step opens the capture directory read-only; all
  outputs go to new locations.
...
| Artifact | Location |
|---|---|
| Raw captures (immutable input) | `/mnt/qnap01/mouse9911/rovers_august_2026/aug1/` — 204 GB, 30 stores |
| Merged DF datasets | `/mnt/qnap01/mouse9911/rovers_august_2026/merged/` — 14 datasets, ~113 GB |
| Segmentation cache (v3.7) | `/mnt/qnap01/mouse9911/precomputed/precompute_cache_3p7/` — 2.0 GB |
| Figures (47) | [`2026_08_01_figures/`](2026_08_01_figures/) — 21 MB |
| Metrics backing the DF table | [`2026_08_01_metrics.json`](2026_08_01_metrics.json) |
| Full pipeline (all 4 steps) | [`tools/run_pipeline.sh`](tools/) |
| Merge | [`spf/scripts/v7_tx_rx_merge.py`](../../../../spf/scripts/v7_tx_rx_merge.py) |
| Precompute | [`spf/scripts/segment_zarr.py`](../../../../spf/scripts/segment_zarr.py) |
| Figure generation | [`tools/generate_figures.sh`](tools/), `zarr_summary_figure.py`, `merged_df_figure.py` |
| DF quality / signal metrics | [`tools/df_metrics.py`](tools/) |
