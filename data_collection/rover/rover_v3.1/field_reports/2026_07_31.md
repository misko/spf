...
Implementation update, 2026-08-01: the production launcher now supplies a
durable status path to the collector. It atomically publishes lifecycle state,
per-receiver counts, common progress, elapsed time, rate, ETA and the primary
error at a bounded cadence. A projected duration over 120% of the measured
1.8-frame/s expectation is marked late after a 30-second grace period. A
nonzero collector exit is persisted, the failure tune is played three times,
and the systemd service remains failed. Focused status/finalization/launcher
tests pass; the complete subprocess interruption file will be repeated after
the active hardware soak because three fixed 10-second startup gates timed out
under concurrent Pi load.
...
Implementation update, 2026-08-01: `python -m
spf.scripts.recover_interrupted_v7` implements the read-only workflow and
refuses naturally complete sources or pre-existing destinations. It validates
each row's timestamp, direct-USB sequence, gain/RSSI metadata and full IQ,
copies only the independently detected aligned prefix, records immutable source
provenance and strictly validates the new V7 store before promotion. A real
SIGTERM artifact and a real unshrunk 128-GiB-logical/5.6-MiB-allocated SIGKILL
LMDB both recovered successfully. The SIGKILL case recovered two frames in
6.83 seconds; its source size, allocated-block count and mtime were unchanged.
The versioned sparse-extent hash avoids reading the 128-GiB hole.
