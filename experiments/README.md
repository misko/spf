...
| [e_agc1_pin_and_detector_bringup](e_agc1_pin_and_detector_bringup/experiment_readme.md) | Do the AD9361 gain-control pins and detector outputs behave the way the tandem-AGC design contract assumes? The mapping is currently verified only by joining a schematic to a constraints file | ✅ session 1 run 2026-08-10 (steps 1–4, 6) — **H1 PASS**: pin map is the identity, 20/20 trials, other channel never moved, so RTL is unblocked. **H2 PASS**. **H6: edges are NOT honoured outside RX** — a real change to the contract. Unplanned: arming takes gain ownership from software, silently (rc=0). Step 5 detectors not yet run (~2–2.5 h, needs RF) |
| [e_gsc6_equal_gain_diagonal](e_gsc6_equal_gain_diagonal/experiment_readme.md) | Does the equal-gain anchor move with gain index? Every campaign to date measures exactly one equal-gain cell — the anchor itself — but the planned tandem-AGC firmware operates entirely on that diagonal | designed 2026-08-10, revised 2026-08-10 after a bench/code audit, not yet run — config change only, rides along on an ordinary additive-cross run as held-out pairs. 21 gains × 24 LOs × 3 epochs = 8,784 frames, **≈2 h capture for both radios** |
