...
| [e_lnk1_transport_sample_rate](e_lnk1_transport_sample_rate/experiment_readme.md) | How do direct-USB, libiio/USB, IIO-over-RNDIS and IIO-over-Ethernet compare across the sample-rate range on `.18` — in throughput, integrity, metadata and measured phase? | ✅ throughput run 2026-08-07 — Ethernet holds up **exactly** as well as USB and no better: both wall at ~23 MB/s (2.9 MS/s), so the limit is the radio, not the link. Ethernet's apparent 28% lead at production buffer size is a buffer-tuning artifact. **Metric 5 (H3) run 2026-08-11 — PASS on three arms including production direct-USB**: 0.519° between arms against 1.515° fixture repeatability. An initial claim that direct-USB reads ~6 dB hotter than libiio was **retracted** — it was the harness using half the calibration path's DDS amplitude; matched, the paths agree to 0.06 dB. RNDIS blocked on the duplicate-IP hazard (low priority); metric 4 (CPU) outstanding |
...
| [e_gsc6_equal_gain_diagonal](e_gsc6_equal_gain_diagonal/experiment_readme.md) | Does the equal-gain anchor move with gain index? Every campaign to date measures exactly one equal-gain cell — the anchor itself — but the planned tandem-AGC firmware operates entirely on that diagonal | ✅ **run 2026-08-11** on RC17, 8,784 frames, 100% quality-valid. **Separability holds** — `C(g,g)` ≈ 0.5° on both radios with no LNA structure. But `D(g,g)` is **not** zero and is LNA-state structured, so tandem leaves `D(g,g)=A(g)` and the residual model stays required. Measured threshold from the clean unit: **7.2× / 8.6× / 2.3×** vs the 6.65° baseline (projected 6.0/5.3/3.4) — worse than projected above 4 GHz. Harness health dominates: the connector-damaged unit is 5–7× worse |
| [e_gsc7_mixer_ladder_high_band](e_gsc7_mixer_ladder_high_band/experiment_readme.md) | Does every adjacent high-band mixer index carry a resolvable phase step, and does that ladder transfer across frequency? | ✅ **run 2026-08-12 over standard IIO USB and IP**, 2,040 validated frames. H1 and H5 falsified. Clean R18 reproduces the aggregate 52→62 effect (USB 5.919°, IP 5.405°), but most individual steps are unresolved and the 5300 MHz curve is not portable. |
