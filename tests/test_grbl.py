from spf.grbl.grbl_interactive import GRBLManager, home_pA, home_pB, home_calibration_point, home_bounding_box, Dynamics, Planner
import numpy as np
...
def test_steps_and_steps_inverse():
  dynamics=Dynamics(calibration_point=np.array([0,0]),
          pA=home_pA,
          pB=home_pB,
          bounding_box=[])
...
def test_polygon_contains():
    ...
    shapely_inside=np.array([ shapely_polygon.contains(geometry.Point(point)) for point in points ])
    shapely_end_time=time.time()

    print(plt_end_time-plt_start_time,shapely_end_time-shapely_start_time)

    #check classic way
    inside_classic=[ check_in_range(x[0],0,1) and check_in_range(x[1],0,1) for x in points]

    assert((plt_inside==inside_classic).all())
    assert((shapely_inside==inside_classic).all())

def test_binary_search_edge():
  dynamics=Dynamics(calibration_point=home_calibration_point,
          pA=home_pA,
          pB=home_pB,
