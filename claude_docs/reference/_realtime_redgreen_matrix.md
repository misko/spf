...
| RT8 | spf_dataset.py:602,647 | __len__ = serving_idx = −1; NOT a live-marker (that's ds.realtime attr) but feeds filters' default steps (filters.py:322,325, both EKFs) → range(-1) = silent empty trajectory | MIN | PF/EKF trajectory on realtime ds with steps=None → today returns empty silently; expect explicit error | len raises TypeError for unbounded stream (or returns served count); filters REQUIRE explicit steps when ds.realtime |
| RT9 | wrapper:98,107,140; spf_dataset write_to_idx | RECLASSIFIED: records are write-once (monotone record_idx, no rewrite path) so LRU staleness is NOT a live bug — but immutability is convention, not contract; and lru_cache-on-methods pins dead instances + 8MB samples (memory only, folds into RT5/G6) | GUARD | green-at-birth: second write_to_idx to same (idx,radio) must raise (today silently accepted) | immutability enforced; any future record-update feature must ship with version-keyed caches; memory covered by G6 soak |
