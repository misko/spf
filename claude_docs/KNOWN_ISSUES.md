...
| 44 | 🟡 P2 · safety (partly fixed 2026-08-03) | **RTL / move can hang forever.** `move_to_point` loops on distance with `sleep(0.1)` and **no timeout/abort**. FIXED for the case that bit us: the stall watchdog hands a jammed rover to the operator in MANUAL (ROVER_RUNBOOK §17), and `move_to_home`'s deadline is now monotonic and shared, so it bounds the drive rather than starting after it. STILL OPEN: a rover that keeps moving without ever arriving has no overall deadline, and the watchdog is inert outside GUIDED. | `spf/mavlink/mavlink_controller.py` |
